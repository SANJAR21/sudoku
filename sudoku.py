...
class solver_class():
         ...
         #               output = self.problem.checker(i, j, k)  # Try to input 'K' & This increases the number of attempts
          #              if output == 1:  # if the value is correct, checker will output 1. Otherwise, output is 0.
           #                 self.puzzle[i-1][j-1]=k
            #                break
    def getMinCell(self, puzzle, row, col, square):
        for _ in range(81):
            min = 10
            for i in range(9):
                for j in range(9):
                    ...
                    for k in range(1, 10):
                        if k in row[j]:
                            ...
                            pos_Number.remove(k)
                            continue
                        elif k in square[h_x + int(h_y / 3)]:
                            pos_Number.remove(k)
                            continue
                    if len(pos_Number) < min:
                        min = len(pos_Number)
                        min_x = i
                        min_y = j
                        t = pos_Number
            return [min_x, min_y, t]
    ...
    def solver(self):
        ...
        for _ in range(81):
            min_Coord = self.getMinCell(self.puzzle, row, col, square)
            min_x = min_Coord[0]
            min_y = min_Coord[1]
            pos_Number = min_Coord[2]

            h_x = min_x - min_x % 3
            h_y = min_y - min_y % 3

            for k in pos_Number:
                if self.problem.checker(min_x + 1, min_y + 1, k) == 1:
                    self.puzzle[min_x][min_y] = k
                    col[min_x].append(k)
                    row[min_y].append(k)
                    square[h_x + int(h_y / 3)].append(k)
                    break
